...
function getOrCreateLobby(betAmount) {
    ...
    for (let id in activeRooms) {
        if (activeRooms[id].betAmount === betAmount) {
            activeRoomList.push(activeRooms[id]);
        }
    }

    // Check if there is an active waiting room for this bet amount
    let roomId = null;
    let targetRoom = null;

    for (let r of activeRoomList) {
        if (r.status === 'waiting' && r.currentRound <= 3) {
            targetRoom = r;
            roomId = r.roomId;
            break;
        }
    ...
    // If all existing rooms for this bet are playing, ended, or all 3 rounds are active/running,
    // check if we need to return active room or create a new waiting room.
    // However, if all 3 rounds/rooms are active/playing, enforce the "ጨዋታው በሂደት ላይ ነው፣ ትንሽ ጠብቁ" rule.
    let allBusy = activeRoomList.length > 0 && activeRoomList.every(r => r.status === 'playing' || r.status === 'ended' || r.currentRound > 3);
    
    if (allBusy || (targetRoom && targetRoom.status === 'playing')) {
        return { busy: true, message: 'ጨዋታው በሂደት ላይ ነው፣ ትንሽ ጠብቁ' };
    }
...
function startRoomGame(roomId) {
    ...
    room.status = 'playing'; // Lock the room completely when game starts
    if (room.timer) clearInterval(room.timer);
...
io.on('connection', (socket) => {
    ...
    socket.on('joinLobby', (data) => {
        ...
        let roomResult = getOrCreateLobby(betAmount);
